...
SKILL_SCRIPTS = Path(
    os.environ.get("MUSIC_TO_VIDEO_SCRIPTS", "/Users/user/.agents/skills/music-to-video/scripts")
)
...
def analyze_audio(bgm: Path, project_dir: Path) -> dict:
    ...
    # The optional music-to-video skill provides a richer beat map. Keep the
    # pipeline runnable without it: the director can still tile the whole
    # track and the renderer will keep lyrics/audio in sync.
    duration = probe_duration(bgm)
    audiomap = {
        "version": 1,
        "summary": "Duration-only fallback map; no beat-grid helper installed.",
        "audio": {"duration_sec": duration},
        "tempo": {},
        "phrases": [],
        "hard_stops": [],
        "key_moments": [],
        "energy_phases": [{"start_sec": 0, "end_sec": duration, "energy": "steady"}],
        "silences": [],
        "rolls": [],
        "events": [],
    }
    audiomap_path.write_text(json.dumps(audiomap, indent=2))
    log("beat-grid helper unavailable; using duration-only audio map")
    return audiomap
...
def plan_storyboard(
    ...
    # Drop frames planned past the track end; clamp the chain to duration.
    frames = [fr for fr in frames if fr["span_sec"][0] < duration - 0.5]
    plan["frames"] = frames
    frames[0]["span_sec"][0] = 0.0
    frames[-1]["span_sec"][1] = duration
    for i in range(len(frames) - 1):
        frames[i]["span_sec"][1] = min(frames[i + 1]["span_sec"][0], duration)
    return plan
...
def normalize_frame_dims(html: str, canvas: tuple[int, int]) -> str:
    """The model sometimes authors frames at half the canvas size; scale them back up.

    Rewrites data-width/height to the canvas and injects a matching `zoom` into the
    frame's own #root rule (head styles are dropped when the template is lifted).
    """
    m = re.search(r'data-width="(\d+)"\s+data-height="(\d+)"', html)
    if not m or (int(m.group(1)) == canvas[0] and int(m.group(2)) == canvas[1]):
        return html
    # Keep data-width at the authored size — hyperframes applies it as the root's
    # inline layout size, so percent-positioned children must keep resolving
    # against it. The visual upscale comes from the transform alone.
    z = canvas[0] / int(m.group(1))
    scale_css = f"transform: scale({z:g}); transform-origin: 0 0;"
    html, n = re.subn(r"(#root\s*\{)", rf"\g<1>\n  {scale_css}", html, count=1)
    if n == 0:
        html = re.sub(r'(<div id="root")', rf'\1 style="{scale_css}"', html, count=1)
    log(f"    normalized frame authored at {m.group(1)}x{m.group(2)} -> scale {z:g}")
    return html
...
def write_frame(client: anthropic.Anthropic, frame: dict, brand: dict, canvas: tuple[int, int], project_dir: Path, model: str, instructions: str = "") -> Path:
    ...
    with client.messages.stream(
        ...
        resp = _s.get_final_message()
    html = normalize_frame_dims(strip_html_fence(resp.content[0].text), canvas)
...
def assemble_index(project_dir: Path, audiomap_path: Path) -> None:
    ...
    # Minimal portable assembler for the same HyperFrames composition contract.
    # It is intentionally data-driven so the generated frame files, lyrics, and
    # source audio remain the single source of truth.
    plan = json.loads((project_dir / "plan.json").read_text())
    duration = float(json.loads(audiomap_path.read_text())["audio"]["duration_sec"])
    frame_nodes = []
    for frame in plan["frames"]:
        start, end = (float(value) for value in frame["span_sec"])
        frame_id = html.escape(str(frame["id"]), quote=True)
        frame_nodes.append(
            f'''      <div id="el-{frame_id}" class="frame"
        data-composition-id="{frame_id}"
        data-composition-src="compositions/frames/{frame_id}.html"
        data-start="{start:.3f}" data-duration="{end - start:.3f}"
...
<style>*{{margin:0;padding:0;box-sizing:border-box}}html,body,#root{{width:1080px;height:1920px;overflow:hidden;background:#000}}.frame{{position:absolute;inset:0;width:100%;height:100%}}</style>
</head><body><div id="root" data-composition-id="main" data-start="0"
data-duration="{duration:.3f}" data-width="1080" data-height="1920">
{chr(10).join(frame_nodes)}
      <audio id="el-bgm" src="assets/bgm.mp3" data-start="0" data-duration="{duration:.3f}" data-track-index="11" data-volume="0.9"></audio>
</div><script>window.__timelines=window.__timelines||{{}};window.__timelines.main=gsap.timeline({{paused:true}});</script></body></html>'''
    )
    log("skill assembler unavailable; generated portable HyperFrames index")
