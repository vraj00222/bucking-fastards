...
- **GATE 5 ✅** Website built + revamped to the LIGHT "blue engraving" theme (paper #f6f2e9 / cobalt #1e3a9e / gold #c3941d; Anton display; long.jpg statue hero; catalog-card UI; DTR-00N numbers; liner-notes lyric page with gold mined-line underlines; roster discography grouping with "THE DIFFICULT SECOND ALBUM" tag). Build green, screenshot-verified desktop+mobile. API contract: `/api/sign` spawns run.py and parses stdout markers `STAGE:intel|lyrics|audio|done SLUG:x`, `FACT:…`, `TITLE:…`; `/api/status/[job]` polls. Dark-theme rollback: commit a0d0304.
